...
# ---- DNS — Instruction CNAME ALB OVH ----
output "dns_cname_instruction" {
  description = "CNAME ALB à ajouter dans la zone DNS OVH après le déploiement Helm"
  value       = try(<<-EOT

    ============================================================
     ACTION 1/2 — CNAME applicatif (OVH → ALB)
    ============================================================
     OVH Panel → Domaines → elbassir.com → Zone DNS → Ajouter

     Type         : CNAME
     Sous-domaine : taskmanager
     Cible        : ${data.kubernetes_ingress_v1.taskmanager.status[0].load_balancer[0].ingress[0].hostname}.
     TTL          : 300
    ============================================================

  EOT
  , "Ingress non disponible (cluster en cours de suppression)")
}

# ---- ACM — Instruction de validation DNS ----
output "acm_validation_instruction" {
  description = "CNAME de validation ACM à ajouter dans la zone DNS OVH"
  value       = try(<<-EOT

    ============================================================
     ACTION 2/2 — CNAME validation certificat SSL ACM (OVH)
    ============================================================
     OVH Panel → Domaines → elbassir.com → Zone DNS → Ajouter

%{for dvo in aws_acm_certificate.taskmanager.domain_validation_options~}
     Type         : CNAME
     Sous-domaine : ${replace(dvo.resource_record_name, ".elbassir.com.", "")}
     Cible        : ${dvo.resource_record_value}
     TTL          : 300
%{endfor~}

     Puis relancer : terraform apply  (attend la validation ~2 min)
    ============================================================
...
# ---- ACM — ARN du certificat ----
output "acm_certificate_arn" {
  description = "ARN ACM à coller dans helm/taskmanager/values.yaml (annotation certificate-arn)"
  value       = aws_acm_certificate_validation.taskmanager.certificate_arn
}

# ---- URL finale ----
output "taskmanager_url" {
  description = "URL publique HTTPS de l'application"
  value       = "https://taskmanager.elbassir.com"
}
