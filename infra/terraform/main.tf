...
resource "kubernetes_namespace" "monitoring" {
  ...
  depends_on = [module.eks_nodegroup, helm_release.alb_controller]
}
...
# Utilise null_resource + kubectl pour éviter l'erreur "Failed to construct REST client"
# lors du terraform plan (la CRD PodMonitor n'existe pas avant l'installation de Prometheus)
resource "null_resource" "taskmanager_podmonitor" {
  triggers = {
    # Recrée la ressource si le manifest change
    manifest = sha256(<<-EOT
...
# ============================================================
# DNS — Lecture de l'Ingress pour générer l'instruction CNAME
# ============================================================

data "kubernetes_ingress_v1" "taskmanager" {
  metadata {
...
# ============================================================
# ACM — Certificat SSL pour taskmanager.elbassir.com
# Validation DNS — les CNAMEs sont affichés dans les outputs
# pour être ajoutés manuellement dans la zone DNS OVH.
#
# Workflow :
#   1. terraform apply  → crée le certificat + affiche le
#      CNAME de validation dans acm_validation_instruction
#   2. Ajouter ce CNAME dans OVH (zone DNS)
#   3. terraform apply  → attend la validation ACM (~2 min)
#   4. terraform output acm_certificate_arn → copier l'ARN
#   5. Ajouter l'ARN dans values.yaml puis helm upgrade
# ============================================================

resource "aws_acm_certificate" "taskmanager" {
  domain_name       = "taskmanager.elbassir.com"
  validation_method = "DNS"

  tags = {
    Name = "taskmanager-elbassir-com"
  }
...
# Attend que le certificat soit validé après ajout du CNAME dans OVH
resource "aws_acm_certificate_validation" "taskmanager" {
  certificate_arn = aws_acm_certificate.taskmanager.arn

  timeouts {
    create = "10m"
  }

  depends_on = [aws_acm_certificate.taskmanager]
}
